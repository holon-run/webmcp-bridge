...
rg -q 'command -v <site>-webmcp' "$SKILL_FILE" || fail 'missing link-first check'
rg -q '<site>-webmcp -h' "$SKILL_FILE" || fail 'missing help-first usage'
rg -q '<site>-webmcp <operation> -h' "$SKILL_FILE" || fail 'missing operation help example'
rg -q '<site>-webmcp <operation> field=value' "$SKILL_FILE" || fail 'missing key=value example'
rg -q "<site>-webmcp <operation> '\{" "$SKILL_FILE" || fail 'missing positional JSON example'
